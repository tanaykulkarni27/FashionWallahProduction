...
export default function BannerCarousel({ Banner_Data }: CarouselProps) {
  ...
  return (
    <div className={`relative w-full flex flex-grow aspect-[768/1366]  md:aspect-[1366/768] ${Banner_Data[activeIndex].class}`}>
      {Banner_Data.map((item, index) => {
        return (
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <div className={`m-0 w-full aspect-[768/1366] md:aspect-[1366/768] flex flex-row justify-center items-center ${item.class}`}>
              <div className='relative'>
                <div
                  className='rounded-2xl w-[90vw] aspect-[768/1366] md:aspect-[1366/768] absolute top-0 left-0 overflow-hidden'
                  dangerouslySetInnerHTML={{ __html: item?.offer_text || '' }}
                ></div>
                <img
                  src={item.image.url}
                  className="rounded-2xl w-[90vw] aspect-[768/1366] md:aspect-[1366/768] object-cover object-center overflow-hidden"
